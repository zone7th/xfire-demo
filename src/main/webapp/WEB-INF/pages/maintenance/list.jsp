...
<div class="content-inner">
	...
	<ul class="breadcrumb">
		<%-- <li><a href="<%=request.getContextPath()%>/home">版本发布管理</a> --%>
			<li class="active">
				<c:if test="${env == 'uat'}">UAT服务维护</c:if>
				<c:if test="${env == 'preprod'}">预生产服务维护</c:if>
				<c:if test="${env == 'gray'}">灰度服务维护</c:if>
				<c:if test="${env == 'prod'}">生产服务维护</c:if>
			</li>
		<li class="active">
			<c:if test="${env == 'uat'}">UAT应用维护</c:if>
			<c:if test="${env == 'preprod'}">预生产应用维护</c:if>
			<c:if test="${env == 'gray'}">灰度应用维护</c:if>
			<c:if test="${env == 'prod'}">生产应用维护</c:if>
		</li>
	</ul>
	...
	<div class="container-fluid">
		...
		<div class="block">
			<p class="block-heading">
				<c:if test="${env == 'uat'}">UAT服务维护列表</c:if>
				<c:if test="${env == 'preprod'}">预生产服务维护列表</c:if>
				<c:if test="${env == 'gray'}">灰度服务维护列表</c:if>
				<c:if test="${env == 'prod'}">生产服务维护列表</c:if>
			</p>
			<div class="block-body">
				<form class="form-horizontal"
					action="<%=request.getContextPath()%>/maintenance/${env}/list"
					method="post">
					<div class="row form-group">
						<div class="col-md-offset-1 col-sm-offset-1">
							<label class="col-md-2  col-sm-1 control-label ">系统名称：</label>
							<div class="col-md-3 col-sm-2">
								<input type="hidden" name="env" id="env" value="${env }"/>
				...
				<c:forEach items="${datas.datas}" var="serverMain"
					...
					<tr>
						...
						<td><a href="#"
							...
							title="重启应用">重启</a> <a
							href="<%=request.getContextPath()%>/logfile/list/${serverMain.id}/${env}/maintenance"
							title="查看日志文件列表">日志</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
