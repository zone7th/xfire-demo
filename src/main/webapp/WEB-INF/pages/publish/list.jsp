...
<script src="<%=request.getContextPath()%>/resources/js/publish/list.js"></script>
			...
			<div class="block">
				...
				<div class="block-body">
					<form class="form-horizontal"
						action="<%=request.getContextPath()%>/publish/${env}" method="post">
					...
					<c:forEach items="${datas.datas}" var="task" varStatus="status">
						<tr>
							...
							<td><c:if test="${task.taskStatus == '-1' }">取消</c:if>
								...
								<%-- <c:if test="${task.taskStatus == '50' }">发布成功</c:if> --%>
							</td>
							...
							<sec:authorize ifAnyGranted="ROLE_ADMIN,ROLE_MID,ROLE_OPS">
								<c:if test="${task.taskStatus !='41'}">
									<a href="<%=request.getContextPath() %>/publish/${env}/${task.id}" title="发布"><i class="icon-play"></i></a> 
								</c:if>
								<c:if test="${task.taskStatus !='40' && task.taskStatus !='41' && task.taskStatus !='42' && task.taskStatus !='-1'}">
									<a href="#" onclick="passStatus(${task.id});" title="跳过"><i class="icon-fast-forward"></i></a> 
								</c:if>
							</sec:authorize>
							
							<a href="<%=request.getContextPath() %>/publish/view/${task.id}/${env}" title="详情"><i class="icon-search"></i></a>
